...
module Rails
  module Crud
    class CrudData
      ...
      def initialize(config: CrudConfig.instance, logger: CrudLogger.logger)
        @config = config
        @logger = logger
        @crud_rows = {}
        @crud_cols = {}
      ...
      def load_crud_data
        ...
        unless File.exist?(@config.crud_file_path)
          @logger.warn "CRUD file not found: #{@config.crud_file_path}"
          return false
        end
      ...
      def reload_if_needed
        ...
        if @last_loaded_time.nil? || File.mtime(@config.crud_file_path) > @last_loaded_time
          @logger.info "Reloading CRUD data due to file modification. last_loaded_time = #{@last_loaded_time}"
          load_crud_data
        end
      end
